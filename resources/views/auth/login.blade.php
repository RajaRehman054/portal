...
<body class="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-100 via-blue-50 to-yellow-50">
    <div class="w-full max-w-md mx-auto p-6 bg-white rounded-2xl shadow-xl">
        ...
        <form method="POST" action="{{ route('login.submit') }}" class="space-y-5">
            @csrf
            @if($errors->any())
                <div class="bg-red-100 text-red-800 px-4 py-2 rounded mb-2 text-center">{{ $errors->first() }}</div>
            @endif
            <div>
            ...
            <div class="flex flex-col items-center mt-4">
                <a href="{{ route('register') }}" class="text-indigo-500 hover:underline text-sm">Don't have an account? Register here</a>
            </div>
            <p class="text-xs text-gray-400 text-center mt-6">&copy; Job Portal {{ date('Y') }}</p>
        </form>
    </div>
</body>
</html>
