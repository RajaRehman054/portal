...
<?php $__env->startSection('navbar-links'); ?>
    <a href="<?php echo e(route('dashboard')); ?>" class="text-white/90 hover:text-white px-3 py-2 rounded transition font-medium <?php echo e(request()->routeIs('dashboard') ? 'bg-white/10' : ''); ?>">Home</a>
    <a href="<?php echo e(route('jobs.index')); ?>" class="text-white/90 hover:text-white px-3 py-2 rounded transition font-medium <?php echo e(request()->routeIs('jobs.index') ? 'bg-white/10' : ''); ?>">View Jobs</a>
    <?php if(session('user') && session('user')->UserType === 'jobseeker'): ?>
        <a href="<?php echo e(route('jobseeker.applications')); ?>" class="text-white/90 hover:text-white px-3 py-2 rounded transition font-medium <?php echo e(request()->routeIs('jobseeker.applications') ? 'bg-white/10' : ''); ?>">My Applications</a>
    <?php endif; ?>
    ...
    <a href="<?php echo e(route('logout')); ?>" class="text-white/90 hover:text-white px-3 py-2 rounded transition font-medium">Logout</a>
...
<div class="max-w-7xl mx-auto px-4 flex">
    <div class="flex-1">
        ...
        <div class="relative flex flex-col items-center justify-center py-16 px-4 text-center mb-10">
            ...
            <div class="relative max-w-2xl w-full mx-auto bg-white/60 backdrop-blur-md rounded-3xl shadow-2xl border-2 border-gradient-to-br from-indigo-400 via-blue-200 to-pink-200 p-10 flex flex-col items-center">
                ...
                <p class="text-lg md:text-xl text-gray-700 mb-2 mt-2">Hello, <span class="font-semibold text-indigo-700"><?php echo e(session('user')->Name); ?></span>! <span class="inline-block align-middle">👋</span></p>
                <p class="text-gray-600 mb-6">Connecting top talent with the best companies worldwide.</p>
                <a href="<?php echo e(route('jobs.index')); ?>" class="inline-block bg-gradient-to-r from-pink-500 via-indigo-500 to-blue-500 hover:from-indigo-600 hover:to-pink-500 text-white font-bold px-8 py-3 rounded-xl shadow-lg transition text-lg tracking-wide">Explore Job Openings</a>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mb-10">
            ...
            <div class="rounded-xl bg-white shadow-lg p-6 flex flex-col items-center hover:shadow-2xl transition group border-t-4 border-indigo-500">
                ...
                <div class="text-3xl font-bold text-indigo-700 mb-1"><?php echo e($totalJobs ?? '—'); ?></div>
            ...
            <div class="rounded-xl bg-white shadow-lg p-6 flex flex-col items-center hover:shadow-2xl transition group border-t-4 border-blue-500">
                ...
                <div class="text-3xl font-bold text-blue-700 mb-1"><?php echo e($totalApplications ?? '—'); ?></div>
            ...
            <div class="rounded-xl bg-white shadow-lg p-6 flex flex-col items-center hover:shadow-2xl transition group border-t-4 border-purple-500">
                ...
                <div class="text-3xl font-bold text-purple-700 mb-1"><?php echo e($totalEmployers ?? '—'); ?></div>
    ...
    <div class="hidden lg:flex flex-col w-80 ml-8 mt-2 sticky top-24 h-[32rem] px-2">
        <div x-data="{
            jobs: <?php echo e(json_encode($highlightJobs)); ?>,
            start: 0,
            visible: 3,
            interval: null,
            next() {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\job_portal\resources\views/dashboard.blade.php ENDPATH**/ ?>